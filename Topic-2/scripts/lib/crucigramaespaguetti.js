    ...
    function DatosJson(datosCrucigrama) {
        ...
        $('.btnComprobar-cruc').click(function (e) {
            ...
            if (veces<4){
                ...
                /*$(".retroalimentaciones").html("<p class='color-negro'>Please, check the correct answers.</p>");                */
                $('.retroalimentaciones p').append("Please, check the correct answers.");
                $('.retroalimentaciones').removeClass("color-rojo");
                $('.retroalimentaciones').addClass("color-negro");
            }
            /*NUEVO TERMINA*/
        });
    ...
    function corregir() {
        ...
        if (vacio) {
            ...
            if (error === 0) {
                ...
                $('.retroalimentaciones p').append("Very good! You are ready to continue.");
                $('.retroalimentaciones').removeClass("color-rojo");
                $('.retroalimentaciones').addClass("color-verde");
                $(".btnlimpiar-cruc").attr('disabled', true);
                $(".btnlimpiar-cruc").removeClass("btn-descarga");
            } else {
                /*$(contenidoVentana).html("<p class='color-rojo'>Try to answer the exercise again.</p>");*/
                $('.retroalimentaciones p').append("Try to answer the exercise again.");
                $('.retroalimentaciones').addClass("color-rojo");
            }
    ...
    $("#"+name+" .comprobar").click(function() {
       ...
       $("#" + name +" input").each(function(i){
            ...
            if( text.toLowerCase().replace(/^[ '"]+|[ '"]+$|( ){2,}/g, '$1').trim() == correctas[i] ){ 
                $(this).siblings(".contretros").html(text +' <i class="result fa-solid fa-check" aria-hidden="true"></i><span class="respmens sr-only">Correct!</span>');
                $(this).siblings(".contretros").addClass("correcto");
                contadorCorrectas++;
            }
            else{
              $(this).siblings(".contretros").html(text +' <i class="result fa-solid fa-xmark" aria-hidden="true"></i><span class="respmens sr-only">Incorrect!</span>');
